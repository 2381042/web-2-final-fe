...
const EditTripod = () => {
  ...
  useEffect(() => {
    const fetchTripod = async () => {
      try {
        const response = await axios.get(`/tripod/${id}`);
        setFormData(response.data);
      } catch (error) {
        toast.error("Failed to fetch tripod data");
        console.error(error);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.patch(`/tripod/${id}`, formData);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Edit Tripod</h1>
